...
function _simple(output,d3,drag,url_list,invalidation)
{
  // 指定圖表的尺寸。
  const width = 1000;
  const height = 400;

  // 調整作業成績  參考實驗室同學邱凡洺的code
  const simple = { ...output };
  simple.children = [output.children[5]];
  let score_2 = [8, 8, 10, 10, 10, 8, 10, 10]
  for (let i = 0; i < 8; i++) {
    simple.children[0].children[i].Hw2_score = score_2[i];
  }
  ...
  const customColors = ["#696773", "#D8D030", "#698E59"];
...
export default function define(runtime, observer) {
  ...
  main.variable(observer("simple")).define("simple", ["output","d3","drag","url_list","invalidation"], _simple);
